...
Participants are restricted to use the pretrained  **ViT-L/14** variant of **CLIP**. Participants are free to use both encoders or only the image encoder, depending on their approach. Code to import and test the model is provided in `export_clip_to_onnx.py` (in the provided example, we exclusively use the image encoder). The official source code to load CLIP is provided [HERE](https://github.com/openai/CLIP) and the pretrained ViT-L/14 is provided [HERE](https://openaipublic.azureedge.net/clip/models/b8cca3fd41ae0c99ba7e8951adf17d267cdb84cd88be6f7c2e0eca1737a03836/ViT-L-14.pt). Please strictly follow their terms of use and licenses. 
